...
export function DataTableSelector({
  ...
  const [pricingLoaded, setPricingLoaded] = useState(true) // Always true since we're using hardcoded values
  ...
  useEffect(() => {
    const cost = selectedTables.reduce((sum, tableName) => {
      const tablePrice = AVAILABLE_DATA_TABLES.find((t) => t.name === tableName)?.cost || 0
      return sum + tablePrice
    }, 0)
    setTotalCost(cost)
    onSelectionChange(selectedTables, cost)
  }, [selectedTables, onSelectionChange]) // Removed currentPricing dependency
  ...
  const getTableCost = (tableName: string) => {
    return AVAILABLE_DATA_TABLES.find((t) => t.name === tableName)?.cost || 0
  }
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Select which data tables to fetch from CheckCarDetails API. Pricing shown is based on current rates.
            </CardDescription>
          </div>
