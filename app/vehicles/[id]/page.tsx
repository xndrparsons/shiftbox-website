...
import { CustomerVehicleDetails } from "@/components/CustomerVehicleDetails"
...
interface Vehicle {
  ...
  ccd_vehicleregistration_vehicleAge?: number
  ...
  ccd_vehicleregistration_tax_days?: number
  ...
  ccd_mileage_summary_lastRecordedMileage?: number
  ...
  ccd_vehiclevaluation_Mileage?: number
...
export default async function VehicleDetailPage({ params }: { params: { id: string } }) {
  ...
  console.log("[v0] Vehicle detail page - looking for vehicle ID:", id)
  ...
  const { data: vehicle, error } = await supabase
    ...
    .select(`
      *,
      ...
      ccd_vehiclevaluation_ValuationList,
      ccd_vehiclevaluation_VehicleDescription,
      ccd_ukvehicledata,
      ccd_vehicleimage_VehicleImages
    `)
    .eq("id", id)
    .single()

  console.log("[v0] Vehicle query result:", { vehicle: vehicle ? "found" : "not found", error })

  if (error) {
    console.log("[v0] Vehicle query error:", error)
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8">
                ...
                <CardContent className="space-y-3">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{vehicle.ccd_vehicleregistration_tax_taxStatus || "N/A"}</p>
                    </div>
                    <div>
                      <span className="text-sm text-muted-foreground">MOT Status:</span>
                      <p className="font-medium">{vehicle.ccd_vehicleregistration_mot_motStatus || "N/A"}</p>
                    </div>
                    <div>
                      <span className="text-sm text-muted-foreground">MOT Expiry:</span>
                      <p className="font-medium">{vehicle.ccd_vehicleregistration_mot_motDueDate || "N/A"}</p>
                    </div>
                    {vehicle.ccd_vehicleregistration_engineCapacity && (
                      <div>
                        <span className="text-sm text-muted-foreground">Engine Capacity:</span>
                        <p className="font-medium">{vehicle.ccd_vehicleregistration_engineCapacity}cc</p>
                      </div>
                    )}
                    {vehicle.ccd_vehicleregistration_euro_status && (
                      <div>
                        <span className="text-sm text-muted-foreground">Euro Status:</span>
                        <p className="font-medium">{vehicle.ccd_vehicleregistration_euro_status}</p>
                      </div>
                    )}
                  </div>
                </CardContent>
        ...
        {/* Comprehensive Vehicle Information Section */}
        <div className="mt-12">
          <CustomerVehicleDetails vehicleId={id} vehicleData={vehicle} />
        </div>
      </div>
    </div>
  )
